...
/*
  Всем привет, хотел бы немного объяснить условие доп задачи (калькулятора)

Задача заключается в том, чтобы сделать аналог калькулятора но в консоли. Порядок должен быть таким:
Введите число
Введите операцию
Введите число
Введите операцию
Введите число
Введите операцию
и так далее, пока пользователь не нажмёт получить результат (операция =). Результат всех введённых операций
Например: 5 + 5 - 10 + 10 + 5 = 15

Таким образом каждая новая операция в качестве первого числа принимает результат предыдущей операции
- Сложение
- Вычитание
- Деление
- Умножение
